<template>
  <el-dialog v-model="dialogVisible" title="查看记忆" width="700px" :close-on-click-modal="false">
    <div v-loading="loading">
      <template v-if="detail">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="记忆主题">{{ detail.theme_name }}</el-descriptions-item>
        ...
        <div class="file-item">
          <el-icon><Document /></el-icon>
          <span>{{ detail.memory_id }}.md</span>
          <el-button type="primary" size="small" link @click="downloadMd">下载</el-button>
        </div>
      </template>
    </div>
...
const statusText = computed(() => {
  const map: Record<string, string> = { processing: '解析中', completed: '已完成', editing: '编辑中' }
...
const fetchDetail = async () => {
  ...
  loading.value = true
  try {
    const res = await getMemoryDetail(props.memoryId)
...
const downloadMd = () => { if (detail.value) ElMessage.info(`下载: ${detail.value.memory_id}.md`) }

watch(() => props.visible, (v) => { if (v) fetchDetail() })
</script>

<style scoped>
.section-title { font-size: 16px; font-weight: 600; color: #303133; margin: 16px 0 8px; padding-bottom: 8px; border-bottom: 1px solid #ebeef5; }
.file-list { margin-bottom: 12px; }
.file-item { display: flex; align-items: center; gap: 8px; padding: 8px 12px; border: 1px solid #ebeef5; border-radius: 6px; margin-bottom: 6px; }
</style>